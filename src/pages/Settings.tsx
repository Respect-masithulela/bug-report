...
import {
  ...
  Mail, Trash2, Shield, Send, AlertTriangle, Loader2, Camera, Code2, Copy, Check, Download
} from "lucide-react";
...
function WidgetTab() {
  const [widgetKey, setWidgetKey] = useState("asm-lab-web");
  const [copied, setCopied] = useState(false);

  const origin = typeof window !== "undefined" ? window.location.origin : "";
  const endpoint = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/bug-intake`;

  const snippet = `<script
  src="${origin}/asm-lab-widget.js"
  data-endpoint="${endpoint}"
  data-widget-key="${widgetKey}"
  data-accent="#6d5efc"
  data-label="Report a bug"
></script>`;
  ...
  return (
    <div className="divide-y divide-border">
      <div className="px-4 md:px-6 py-4">
        <p className="text-[13px] font-medium mb-1">Install the ASM Lab bug widget</p>
        <p className="text-[12px] text-muted-foreground max-w-xl">
          Paste this snippet before the closing <code>&lt;/body&gt;</code> tag of ASM Lab. It adds a floating
          “Report a bug” button, keeps the user's last 10 actions in memory, and sends them here with the report.
          Nothing is transmitted until a user submits.
        </p>
      </div>

      <div className="px-4 md:px-6 py-4 space-y-1 max-w-lg">
        <Label className="text-[12px] text-muted-foreground">Widget key</Label>
        <Input
          value={widgetKey}
        ...
        <p className="text-[11px] text-muted-foreground">
          Stored on each report so you can tell which build or environment it came from.
        </p>
      </div>

      <div className="px-4 md:px-6 py-4">
        <div className="flex items-center justify-between mb-2">
          <Label className="text-[12px] text-muted-foreground">Embed snippet</Label>
        ...
        <pre className="text-[11px] font-mono bg-muted rounded-md p-3 overflow-auto whitespace-pre">{snippet}</pre>
      </div>

      <div className="px-4 md:px-6 py-4">
        <p className="text-[12px] text-muted-foreground font-medium mb-2">What gets captured</p>
        <ul className="text-[12px] text-muted-foreground space-y-1 list-disc pl-4">
          <li>Last 10 actions: clicks, page changes, form submits, focused fields, JS errors</li>
          <li>Browser, OS, screen and viewport size, page URL, locale and timezone</li>
          <li>Optional reporter email — never any typed values, passwords or screenshots</li>
        </ul>
      </div>
    </div>
  );
...
function CodebaseTab() {
  return (
    <div className="divide-y divide-border">
      <div className="px-4 md:px-6 py-4">
        <p className="text-[13px] font-medium mb-1">Download the codebase</p>
        <p className="text-[12px] text-muted-foreground max-w-xl">
          Get a full copy of the ASM Lab bug tracker source code as a compressed archive.
          The archive is refreshed automatically every time the app is built, so it always
          matches what's currently live.
        </p>
      </div>
      <div className="px-4 md:px-6 py-4">
        <Button asChild size="sm" className="h-8 text-[12px]">
          <a href="/asm-lab-codebase.tar.gz" download="asm-lab-codebase.tar.gz">
            <Download className="h-3.5 w-3.5 mr-1.5" /> Download codebase (.tar.gz)
          </a>
        </Button>
        <p className="text-[11px] text-muted-foreground mt-2">
          Open it with any archive tool (7-Zip, WinRAR, or built-in on Mac/Linux).
        </p>
      </div>
    </div>
  );
          ...
          <Tabs defaultValue="profile" className="flex flex-col md:flex-row h-full">
            <div className="md:w-44 shrink-0 border-b md:border-b-0 md:border-r border-border">
              <TabsList className="flex md:flex-col items-stretch w-full bg-transparent h-auto p-1.5 gap-px">
                ...
                <TabsTrigger value="widget" className="justify-start gap-1.5 text-[12px] h-7 px-2 data-[state=active]:bg-muted w-full">
                  <Code2 className="h-3.5 w-3.5" /> Widget
                </TabsTrigger>
                <TabsTrigger value="codebase" className="justify-start gap-1.5 text-[12px] h-7 px-2 data-[state=active]:bg-muted w-full">
                  <Download className="h-3.5 w-3.5" /> Codebase
                </TabsTrigger>
            ...
            <div className="flex-1 min-w-0">
              ...
              <TabsContent value="widget" className="m-0"><WidgetTab /></TabsContent>
              <TabsContent value="codebase" className="m-0"><CodebaseTab /></TabsContent>
