...
export default function BugDetail() {
  ...
  const fetchEvents = async () => {
    if (!id) return;
    const { data } = await supabase
      .from("bug_events").select("*").eq("bug_id", id).order("sequence", { ascending: true });
    setEvents(data || []);
  };
        ...
        <div className="flex-1 overflow-auto">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-1 min-w-0 border-r border-border">
              ...
              <SessionContextPanel
                events={events}
                context={(bug.client_context as Record<string, string> | null) ?? null}
              />
            ...
            <div className="w-full lg:w-64 shrink-0">
              ...
              <div className="px-4 py-3 space-y-3 text-[13px]">
                <div>
                  <p className="text-[12px] text-muted-foreground mb-0.5">Reported via</p>
                  <p>
                    {bug.source === "widget" ? "In-product widget" : "Internal"}
