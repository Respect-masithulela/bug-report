...
export const navItems = [
  ...
  { icon: Plus, label: "Report Bug", path: "/bugs/new", staffOnly: false },
  { icon: Bug, label: "My Bugs", path: "/bugs", staffOnly: false },
  { icon: BarChart3, label: "Analytics", path: "/analytics", staffOnly: true },
  { icon: Settings, label: "Settings", path: "/settings", staffOnly: true },
];
...
export function SidebarContent({ collapsed = false, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
  ...
  const items = navItems.filter((i) => !i.staffOnly || isStaff);
